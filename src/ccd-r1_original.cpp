...
inline double RankOneUpdate(const smat_t_Double &R, const int j, const vec_t_Double &u, const double lambda, const double vj, double *redvar, int do_nmf){
	double g = 0, h = lambda;
...
inline double UpdateRating(smat_t_Double &R, const vec_t_Double &Wt, const vec_t_Double &Ht, const vec_t_Double &oldWt, const vec_t_Double &oldHt) {
	double loss = 0;
...
inline double UpdateRating(smat_t_Double &R, const vec_t_Double &Wt2, const vec_t_Double &Ht2) {
	double loss = 0;
#pragma omp parallel for schedule(kind) reduction(+:loss)
	for (int c = 0; c < R.cols; ++c){
...
inline double UpdateRating(smat_t_Double &R, const vec_t_Double &Wt, const vec_t_Double &Ht, bool add) {
	double loss = 0;
	if (add) {
#pragma omp parallel for schedule(kind) reduction(+:loss)
		for (int c = 0; c < R.cols; ++c){
