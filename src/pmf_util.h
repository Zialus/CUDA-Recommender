...
class SparseMatrix {
public:
    long rows, cols, nnz, max_row_nnz_, max_col_nnz_;
    ...
    void read_binary_file(const std::string& fname_csr_row_ptr, const std::string& fname_csr_col_indx,
        ...
        this->read_compressed(fname_csr_row_ptr, fname_csr_col_indx, fname_csr_val,
                              this->csr_row_ptr_, this->csr_col_indx_, this->csr_val_, this->rows + 1,
                              this->max_row_nnz_);

        /// read csc
        this->read_compressed(fname_csc_col_ptr, fname_csc_row_indx, fname_csc_val,
                              this->csc_col_ptr_, this->csc_row_indx_, this->csc_val_, this->cols + 1,
                              this->max_col_nnz_);
    }

    void initialize_matrix(long rows_, long cols_, long nnz_) {
        this->rows = rows_;
        this->cols = cols_;
        this->nnz = nnz_;

        /// alloc csr
        this->alloc_space(this->csr_row_ptr_, this->csr_col_indx_, this->csr_val_, this->rows + 1);

        /// alloc csc
        this->alloc_space(this->csc_col_ptr_, this->csc_row_indx_, this->csc_val_, this->cols + 1);
    }
...
//    fread(&cs_ptr.get()[0], sizeof(unsigned) * num_elems_in_cs_ptr, 1, f_val);
    ...
    void alloc_space(std::shared_ptr<unsigned>& cs_ptr, std::shared_ptr<unsigned>& cs_indx,
        ...
        cs_val = std::shared_ptr<DTYPE>(new DTYPE[this->nnz], std::default_delete<DTYPE[]>());
    }
...
public:
    ...
    void read(long rows_, long cols_, long nnz_, const std::string& filename) {
        ...
        test_val = std::unique_ptr<DTYPE[]>(new DTYPE[nnz_]);
    ...
    void read_binary_file(long rows_, long cols_, long nnz_,
        ...
        CHECK_FREAD(fread(&test_val.get()[0], sizeof(DTYPE) * this->nnz, 1, f_val), 1);
