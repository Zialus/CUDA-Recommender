...
__global__ void updateW_overH_kernel(long rows, const long* row_ptr, const long* col_idx, const long* colMajored_sparse_idx, const float* val, float lambda, unsigned k, float* W, float* H);

__global__ void updateH_overW_kernel(long cols, const long* col_ptr, const long* row_idx, const float* val, float lambda, unsigned k, float* W, float* H);
